...
const getAppRoot = (
    ...
    searchFrom ??= import.meta.dirname
...
type ClassConstructor<T = unknown> = ( new ( ...args: unknown[] ) => T ) & object;
...
/**
 * Given a class, return the names of both its instance and static non-method properties
 * @param cls - The class to get the property names for
 */
function getNonMethodPropertyNames<T>( cls: ClassConstructor<T> ): { instanceProperties: string[], staticProperties: string[] } {
    const instanceProperties = new Set<string>()
    const staticProperties = new Set<string>()
    const ignoreStatic = new Set() // new Set( [ "length", "name", "prototype" ] )

    // Traverse the prototype chain for instance properties
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    let currentPrototype = cls.prototype
    while ( currentPrototype && currentPrototype !== Object.prototype ) {
        const prototypePropertyNames = Object.getOwnPropertyNames( currentPrototype )
    ...
    // Traverse the prototype chain for static properties
    let currentClass: unknown = cls
    while ( currentClass && currentClass !== Function.prototype ) {
        const staticPropertyNames = Object.getOwnPropertyNames( currentClass )
        staticPropertyNames.forEach( name => {
            const descriptor = Object.getOwnPropertyDescriptor( currentClass, name )
            if ( descriptor && typeof descriptor.value !== "function" && !ignoreStatic.has( name ) ) {
                staticProperties.add( name )
            }
        } )
    ...
    return {
        instanceProperties : Array.from( instanceProperties ),
        staticProperties   : Array.from( staticProperties )
    }
}

export { getAppRoot, getAppCallStack, sleep, copyDirectory, isAsyncFunction, asyncEvery, getMethodNames, getNonMethodPropertyNames }
